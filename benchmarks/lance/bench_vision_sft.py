...
def _measure_iter(ds, *, batch_size, num_workers, num_batches, warmup):
    ...
    loader = torch.utils.data.DataLoader(
        ds, batch_size=batch_size, num_workers=num_workers, collate_fn=_collate, drop_last=True,
...
def _measure_map(ds, *, batch_size, num_workers, num_batches, warmup):
    ...
    loader = torch.utils.data.DataLoader(
        ds, batch_size=batch_size, sampler=sampler, num_workers=num_workers, collate_fn=_collate, drop_last=True,
        persistent_workers=num_workers > 0, prefetch_factor=4 if num_workers > 0 else None,
        multiprocessing_context="spawn" if num_workers > 0 else None,
    )
    seen, t0 = 0, None
...
def _side_entry(side, workers, a, q):
    ...
    if side == "base":
        ...
        sps = _measure_iter(ds, batch_size=a["batch_size"], num_workers=workers,
                            num_batches=a["num_batches"], warmup=a["warmup"])
    else:
        ds = build_lance(a["uri"], tokenize, region=a["region"], table=a["table"])
        sps = _measure_map(ds, batch_size=a["batch_size"], num_workers=workers,
                           num_batches=a["num_batches"], warmup=a["warmup"])
    q.put(sps)
    q.close()
    q.join_thread()
...
def main():
    ...
    print(f"mode={args.mode} regime={regime} batch_size={args.batch_size} "
    ...
    for workers in args.num_workers:
        ...
        print(f"{workers:>8}{sps.get('base', float('nan')):>12.1f}{sps.get('lance', float('nan')):>12.1f}{spd:>9.2f}x", flush=True)
