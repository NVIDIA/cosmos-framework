...
"""``vision_sft_nano_v2`` — Cosmos3-Nano vision SFT recipe (dataflow-loader mirror).

Identical to ``vision_sft_nano`` except:
  - ``job.name`` is ``vision_sft_nano_v2``
  - ``dataloader_train`` uses the new four-role CosmosDataLoader stack:
      RankPartitionedDistributor → IdentityProcessor → SequentialPackingBatcher
      → VFMListCollator

All other blocks (optimizer, scheduler, trainer, model, checkpoint, ema,
callbacks, defaults) are byte-identical to ``vision_sft_nano``.
"""
...
for _item in [vision_sft_nano_v2]:
