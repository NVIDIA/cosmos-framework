...
"""Lightweight AgiBot GEAR forward kinematics for datasets and viewers."""
...
from cosmos_framework.data.vfm.action.agibot_gear_spec import (
    ...
    get_agibot_gear_embodiment_spec,
    get_agibot_gear_kind_spec,
...
def _apply_ext_base_motion_to_poses(
    ...
    if embodiment_type != "agibot_gear_gripper_ext":
        return poses_by_name
    if states.shape[1] < AGIBOT_GEAR_EXT_STATE_ROBOT_ORIENTATION_SLICE.stop:
        raise ValueError(
            f"agibot_gear_gripper_ext state must include robot pose through index "
...
def _get_agibot_gear_mujoco_kinematics_xml() -> str:
    """Build a MuJoCo-loadable kinematics-only XML string from the committed URDF."""

    root = ET.parse(get_agibot_gear_urdf_path()).getroot()
...
class _MujocoFk:
    ...
    def __init__(self) -> None:
        ...
        self._mujoco = mujoco
        self.model = mujoco.MjModel.from_xml_string(_get_agibot_gear_mujoco_kinematics_xml())
...
def _set_gripper_joint_values_from_state(
    ...
    embodiment_spec = get_agibot_gear_embodiment_spec(embodiment_type)
    ...
    if embodiment_type == "agibot_gear_gripper_ext":
        ...
        kind_spec = get_agibot_gear_kind_spec(embodiment_type)
