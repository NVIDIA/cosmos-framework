...
"""Shared AgiBot GEAR metadata used by datasets and visualizers."""
...
AGIBOT_GEAR_EMBODIMENT_SPECS: dict[str, AgibotGearEmbodimentSpec] = {
    "agibot_gear_gripper": AgibotGearEmbodimentSpec(
        embodiment_type="agibot_gear_gripper",
        kind="gripper",
    ),
    "agibot_gear_gripper_ext": AgibotGearEmbodimentSpec(
        embodiment_type="agibot_gear_gripper_ext",
        kind="gripper",
    ),
}


def get_agibot_gear_embodiment_spec(embodiment_type: str) -> AgibotGearEmbodimentSpec:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown AgiBot GEAR embodiment_type={embodiment_type!r}. "
            f"Expected one of {sorted(AGIBOT_GEAR_EMBODIMENT_SPECS)}."
        ) from exc


def get_agibot_gear_kind_spec(embodiment_type: str | AgibotGearKind) -> AgibotGearKindSpec:
    """Resolve an embodiment type or kind to its shared layout metadata."""

    kind = embodiment_type if embodiment_type in AGIBOT_GEAR_KIND_SPECS else get_agibot_gear_kind(embodiment_type)
    return AGIBOT_GEAR_KIND_SPECS[kind]


def get_agibot_gear_kind(embodiment_type: str) -> AgibotGearKind:
    """Return the hand kind used by one AgiBot embodiment."""

    return get_agibot_gear_embodiment_spec(embodiment_type).kind


def get_agibot_gear_urdf_path() -> Path:
    """Return the committed AgiBot GEAR G1 omnipicker URDF path."""
