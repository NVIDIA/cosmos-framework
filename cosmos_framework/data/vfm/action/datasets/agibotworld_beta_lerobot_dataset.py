...
def _assemble_agibot_gear_state(
    effector_pos: np.ndarray,
    joint_pos: np.ndarray,
    head_pos: np.ndarray,
    waist_pos: np.ndarray,
) -> np.ndarray:
    """Assemble standard 20D GEAR gripper state from Beta decomposed fields."""
...
class AgiBotWorldBetaLeRobotDataset(Dataset):
    ...
    def _build_fk_action(self, rows: list[dict[str, Any]]) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        states_np = _assemble_agibot_gear_state(effector_pos, joint_pos, head_pos, waist_pos)

        native_fk = compute_fk_transforms_batch(states_np, "agibot_gear_gripper")
        native_fk = apply_robot_base_motion_to_poses(native_fk, robot_pos, robot_quat)
        fk = apply_agibot_gripper_to_opencv(native_fk, AGIBOT_GEAR_GRIPPER_TO_OPENCV_BY_WRIST)
