...
def _pad_to_size(size: int, x: torch.Tensor) -> torch.Tensor:
    assert x.shape[0] <= size
    padded = x.new_zeros((size, *x.shape[1:]))
    padded[: x.shape[0]] = x
    return padded
...
def sequence_pack_from_packed_sequence(
    ...
    need_causal = _get_padded_size(len_causal, cp_world_size, pad_for_cuda_graphs)
    ...
    if pad_segment_supported and (need_causal > len_causal or need_full > len_full):
        need_causal = max(need_causal, _get_padded_size(len_causal + 1, cp_world_size, pad_for_cuda_graphs))
        need_full = max(
