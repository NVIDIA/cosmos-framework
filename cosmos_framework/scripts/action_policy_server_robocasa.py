...
"""HTTP inference server for RoboCasa action policies using OmniMoTModel.

Mirrors ``action_policy_server_libero`` (same endpoints, same request format) with the
additions the RoboCasa recipe needs: an optional ``"state"`` field carrying the current
end-effector pose, which is prepended to the action sequence as the clean conditioning
frame at index 0 for checkpoints trained with ``use_state=True``.
...
Example:
  ...
  PYTHONPATH=. python -m cosmos_framework.scripts.action_policy_server_robocasa \
...
# Viewpoint tag for the concat_view (third-person + wrist) composite the client sends;
# matches the dataset's _VIEWPOINT_BY_CAMERA["concat_view"]. Used only when the experiment
# trains with JSON-structured prompts (format_prompt_as_json=True).
_JSON_VIEWPOINT = "concat_view"
...
class ActionModelService:
    def __init__(self, args: ActionServerArgs) -> None:
        ...
        # Whether the checkpoint was trained with proprioception prepended as a clean
        # conditioning action frame. A request must then carry "state": serving such a model
        # without it does not raise anywhere downstream, it just drops a conditioning signal
        # the policy depends on, so the mismatch is rejected explicitly in _prep_policy_item.
        self.requires_state = _extract_bool_from_config(self.experiment_config, "use_state", default=False)
        if self.requires_state:
            log.info("[action-server] checkpoint trained with use_state=True; requests must carry 'state'")
    ...
    def _resolve_action_normalization(
        ...
        raise ValueError(
            "action_policy_server_robocasa.py can denormalize action_normalization='minmax', 'meanstd', "
    ...
    def _build_json_prompt(self, prompt: str, *, video: torch.Tensor, image_size: torch.Tensor) -> str:
        ...
        the zero ``action`` (total-frame count) mirror the RoboCasa concat_view dataset."""
        data_dict: dict[str, Any] = {
            "ai_caption": prompt,
            "viewpoint": _JSON_VIEWPOINT,
    ...
    def _prep_policy_item(self, req: dict[str, Any]) -> dict[str, Any]:
        ...
        # Optional proprioception: a raw ``raw_action_dim``-wide state token (the current eef
        # pose) prepended as the clean conditioning action frame at index 0, matching the
        # ``use_state`` dataset path.
        state_token: torch.Tensor | None = None
        raw_state = req.get("state")
        if raw_state is None and self.requires_state:
            raise ValueError(
                "this checkpoint was trained with use_state=True, so 'state' is required: send the "
                f"current eef pose as a {self.raw_action_dim}-wide list. Omitting it does not fail "
                "downstream, it just drops the conditioning frame the policy was trained on."
            )
    ...
    def _build_action_input(self, state_token: "torch.Tensor | None") -> "torch.Tensor":
        """Zeros placeholder [T, max_action_dim] for the diffusion start. A state token becomes
        row 0, the clean conditioning frame, so T = chunk+1. It is written raw (unnormalized) to
        match ``action_normalization=None``; a normalized-state run would need the forward
        normalizer applied here too."""
        t = self.cfg.action_chunk_size + (1 if state_token is not None else 0)
        action_t_d = torch.zeros((t, self.cfg.max_action_dim), dtype=torch.float32)
        if state_token is not None:
            action_t_d[0, : state_token.shape[0]] = state_token
        return action_t_d
    ...
    def predict_policy_batch(self, reqs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        batch: dict[str, Any] = {
            ...
            **make_batched_action_processing_fields(
            ...
            "action": [[self._build_action_input(p["state_token"])] for p in preps],
            "mode": ["wam"] * n,
        ...
        for i in range(n):
            ...
            if preps[i]["state_token"] is not None:
                pred = pred[1:]  # drop the prepended clean state-conditioning frame
    ...
    def predict_policy(self, req: dict[str, Any]) -> dict[str, Any]:
        ...
        # Action: zeros noise start; row 0 holds the clean state-conditioning token when
        # proprioception was supplied.
        action_t_d = self._build_action_input(prep["state_token"])  # [T,action_dim]
