...
def _build_public_export_model_config(model_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    if quantization is not None:
        ...
        inert_keys = {
            "fp8_granularity",
            "matched_fqns_dump_path",
            "qdq_group_size",
            "sim_edges",
            "sim_residual_group_size",
            "sim_residual_bits",
            "sim_attn_v_block_size",
            "sim_attn_smoothing",
            "sim_attn_v_format",
            "sim_attn_p_format",
            "sim_attn_pv_accum",
            "sim_attn_k_scope",
            "mixed_precision_reasoner_policy",
            "mixed_precision_w8a16_cache",
        }
        comparable = {key: value for key, value in quantization_values.items() if key not in inert_keys}
        if any(
            key not in disabled_quantization or value != disabled_quantization[key] for key, value in comparable.items()
        ):
            raise ValueError(
