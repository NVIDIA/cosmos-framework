    ...
    def forward(
        ...
        if self.flex_backend is not None:
            ...
            attention_meta.flex_block_mask = build_multiview_block_mask(
                ...
                token_shapes=mask_items.token_shapes,
                condition_masks=mask_items.condition_masks,
                num_items_per_sample=mask_items.num_items_per_sample,
                num_views_per_item=mask_items.num_views_per_item,
                ...
                view_offsets_per_item=mask_items.view_offsets_per_item,
            )
...
@dataclass
class _MultiviewMaskItems:
    """One flat item list per field, describing a batch's generation streams to the mask."""

    num_items_per_sample: list[int]
    token_shapes: list[tuple[int, ...]]
    condition_masks: list[torch.Tensor]
    num_views_per_item: list[int]
    view_offsets_per_item: list[int] | None


def _multiview_mask_items(packed_seq: PackedSequence) -> _MultiviewMaskItems:
    ...
    views, and the offset is what keeps the rules that match on ``(frame, view)`` from
    pairing a camera latent with the sweep that happens to share its frame index -- two
    different instants, since the streams run at different latent rates. A batch with no
    LiDAR, or a LiDAR-only batch, reports ``None`` and its mask is bit-identical to the
    single-stream one.
    """
    ...
    condition_masks: list[torch.Tensor] = []
    num_views_per_item: list[int] = []
    view_offsets_per_item: list[int] = []
    vision_cursor = 0
    lidar_cursor = 0
    for sample_idx in range(num_samples):
        if vision is not None:
            for _ in range(vision_counts[sample_idx]):
                token_shapes.append(vision.token_shapes[vision_cursor])
                condition_masks.append(vision.condition_mask[vision_cursor])
                num_views_per_item.append(views_per_vision_item[vision_cursor])
                view_offsets_per_item.append(0)
                vision_cursor += 1
        if lidar is not None:
            for _ in range(lidar_counts[sample_idx]):
                token_shapes.append(lidar.token_shapes[lidar_cursor])
                condition_masks.append(lidar.condition_mask[lidar_cursor])
                num_views_per_item.append(1)
                view_offsets_per_item.append(lidar_view_offset)
                lidar_cursor += 1
        num_items_per_sample.append(vision_counts[sample_idx] + lidar_counts[sample_idx])

    return _MultiviewMaskItems(
        ...
        # Offsets only matter when both streams share a sample. A single-stream
        # pack keeps every item on view 0, the same as the camera-only path.
        view_offsets_per_item=view_offsets_per_item if vision is not None and lidar is not None else None,
    )
