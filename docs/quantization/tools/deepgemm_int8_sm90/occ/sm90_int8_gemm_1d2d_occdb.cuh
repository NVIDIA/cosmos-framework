...
// Generated by kernels/deepgemm_int8/occ/gen_kernel_occ.py from include/deep_gemm/impls/sm90_int8_gemm_1d2d_db.cuh -- do not edit by hand.
...
sm90_int8_gemm_1d2d_occdb_impl(float* sfb, int* grouped_layout,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    DG_STATIC_ASSERT(not kHalfD or kSwizzleDMode > 0, "kHalfD needs a swizzled D store");
    ...
    constexpr uint32_t kNumMathRegisters = kMathRegs;   // (OCC) lowered so kMinBlocksPerSM CTAs fit the register file
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            constexpr uint32_t WAVE_BLOCK_M = BLOCK_M <= WGMMA::M ? BLOCK_M : WGMMA::M * (kNumMathThreads / 128);   // (OCC) waves per warpgroup
            ...
            if (scheduler.is_computation_valid(m_block_idx, math_wg_idx * WGMMA::M)) {
                ...
                dispatch_num_former_iters<0, kGap, kEnd>(kShouldOptimize ? num_former_iters : 0, [&](auto _) {
                  if constexpr (BLOCK_M / WAVE_BLOCK_M == 1 and BLOCK_M >= WGMMA::M) {
                    ...
                    // Straight-line regions of kU k-blocks: buffers alternate inside the region, every group is retired (wait<0>) at the
                    // region end, so no wgmma group is in flight across a loop back-edge; 1 of kU promotions per region is exposed.
                    constexpr uint32_t kU = 8;
            ...
            // (OCC) D tile written and TMA-stored in kNumDPasses passes through a buffer of kAtomsPerPass swizzle atoms (1 pass = the default epilogue)
