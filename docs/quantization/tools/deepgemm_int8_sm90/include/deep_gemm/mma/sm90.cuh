// Copied from DeepGEMM (MIT License, Copyright (c) 2025 DeepSeek); INT8 port adds S8MMA/S8MMASelector.
#pragma once
...
/// (INT8 port) INT8 x INT8 -> INT32 WGMMA: `wgmma.mma_async.sync.aligned.m64nNk32.s32.s8.s8` (K = 32 per instruction like
/// e4m3, same accumulator register count as f32: M*N/128 x 32-bit per thread, no imm-scale-a/b operands, scale-d predicate kept).
/// PTX only allows N in {8, 16, 24, 32, 48, 64, 80, ..., 256} (multiples of 16 above 32) for integer wgmma; CUTLASS ships exactly
/// that set as MMA_64xNx32_S32S8S8_SS_TN (uint32_t& accumulators, reinterpreted from our int32_t array).
template <int N_, typename MMA>
struct S8MMA {
    template <size_t ...Idx>
...
struct S8MMASelector {
    ...
    static constexpr auto select_type() {
        DG_STATIC_ASSERT(N == 8 or N == 16 or N == 24 or N == 32 or (N % 16 == 0 and N <= 256), "Invalid N for INT8 WGMMA");
        return S8MMA<N, decltype(select_mma())>();
    }
