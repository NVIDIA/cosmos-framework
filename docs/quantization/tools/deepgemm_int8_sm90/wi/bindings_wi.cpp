// torch bindings of the wave-interleave experiment extension (module deepgemm_int8_wi_ext); mirrors ../bindings.cpp
#include <torch/extension.h>
#include <ATen/cuda/CUDAContext.h>
#include <c10/cuda/CUDAGuard.h>
#include <vector>
#include "wi_configs.h"

torch::Tensor int8_gemm_g128_wi(torch::Tensor A, torch::Tensor W, torch::Tensor sfa, torch::Tensor sfb,
    ...
    TORCH_CHECK(cfg >= 0 && cfg < kDgInt8WiNumCfgs, "bad config index");
    const auto& info = kDgInt8WiCfgs[cfg];
    ...
    kDgInt8WiRuns[cfg](args);
    return D;
}

std::vector<std::vector<int64_t>> configs() {
    std::vector<std::vector<int64_t>> r;
    for (int i = 0; i < kDgInt8WiNumCfgs; ++i) {
        const auto& c = kDgInt8WiCfgs[i];
        r.push_back({c.block_m, c.block_n, c.stages, c.mcast, c.on_a, c.shape_n, c.shape_k, c.math_threads, c.smem_max /* = wave mode */});
    }
    return r;
}
std::vector<std::string> config_names() {
    std::vector<std::string> r;
    for (int i = 0; i < kDgInt8WiNumCfgs; ++i) r.push_back(kDgInt8WiCfgs[i].name);
    return r;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("int8_gemm_g128_wi", &int8_gemm_g128_wi, "wave-interleave experiment of the DeepGEMM INT8 port (+ reference instantiations)",
