...
// int8_gemm_g128(A[M,K] int8, W[N,K] int8, sfa[K/128, round_up(M,4)] fp32, sfb[N/128, K/128] fp32, out (bf16 [M,N] or None), cfg) -> bf16 [M,N]
torch::Tensor int8_gemm_g128(torch::Tensor A, torch::Tensor W, torch::Tensor sfa, torch::Tensor sfb,
    ...
    TORCH_CHECK(cfg >= 0 && cfg < kDgInt8NumCfgs, "bad config index");
    const auto& info = kDgInt8Cfgs[cfg];
    ...
    kDgInt8Runs[cfg](args);
    return D;
}

std::vector<std::vector<int64_t>> configs() {
    std::vector<std::vector<int64_t>> r;
    for (int i = 0; i < kDgInt8NumCfgs; ++i) {
        const auto& c = kDgInt8Cfgs[i];
        r.push_back({c.block_m, c.block_n, c.stages, c.mcast, c.on_a, c.shape_n, c.shape_k, c.math_threads});
    }
    return r;
}
std::vector<std::string> config_names() {
    std::vector<std::string> r;
    for (int i = 0; i < kDgInt8NumCfgs; ++i) r.push_back(kDgInt8Cfgs[i].name);
    return r;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("int8_gemm_g128", &int8_gemm_g128, "DeepGEMM SM90 1D2D kernel ported to INT8: (1,128,128) block-scaled GEMM, bf16 out",
